...
async def get_switch_interface_detail(host: Host) -> dict:
    """Retrieves description, mac, ip, cidr, vlan and desktop details"""
    result = await host.send_command(["show interfaces", "show vlan", "show mac address-table"])
    result_dict = {}

    async for record in parse_show_interface(result["show interfaces"]):
...
async def parse_show_interface(data: dict):
